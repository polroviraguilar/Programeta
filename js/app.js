// js/app.js — MVP sense Firebase (només LocalStorage)

// ──────────────────────────────────────────
// Estat
// ──────────────────────────────────────────
let selected = currentISO(); // { date, week, year }
let franges = load('franges') || [];
let llicons = load('llicons') || [];
...
function renderWeek(){
  ...
  for (const t of TIMES){
    ...
    for (let i=0;i<DAYS.length;i++){
      const d = DAYS[i]; const key = `${d}|${t}|${selected.year}|${selected.week}`;
      const f = franges.find(x=>x.key===key);
...
function openEdit(key){
  currentEditKey = key;
  currentEditIndex = franges.findIndex(x=>x.key===key);
  ...
  if (currentEditIndex>=0){
    const f = franges[currentEditIndex];
    fcurs.value = f.curs||''; fassignatura.value=f.assignatura||''; factivitat.value=f.activitat||''; ftipus.value=f.tipus;
  }
  dlg.showModal();
}
...
document.getElementById('delFranja').addEventListener('click', ()=>{
  if (currentEditIndex>=0){ franges.splice(currentEditIndex,1); save('franges',franges); }
  dlg.close(); renderWeek();
});

document.getElementById('saveFranja').addEventListener('click', ()=>{
  const base = {
    key: currentEditKey,
    dia: fdia.value, hora: fhora.value,
    curs: fcurs.value.trim(), assignatura: fassignatura.value.trim(),
    activitat: factivitat.value.trim(), tipus: ftipus.value
  };
  if (currentEditIndex>=0) franges[currentEditIndex]=base;
  else franges.push(base);
  save('franges',franges);
  dlg.close(); renderWeek();
});

// ──────────────────────────────────────────
// Vista anual (selector de setmanes)
// ──────────────────────────────────────────
...
document.getElementById('saveLlico').addEventListener('click', ()=>{
  const data = {
  ...
  llicons.unshift(data);
  save('llicons', llicons);
...
function renderLlicons(){
  ...
  const rows = llicons.filter(l=> (!fc||l.curs===fc) && (!fa||l.assignatura===fa));
  ...
  llistaLlicons.innerHTML = rows.map(r=>`
    <div class="item">
      ...
      ${r.recursos.length?`<div class="meta">Recursos: ${r.recursos.map(escapeHtml).join(', ')}</div>`:''}
    </div>
  `).join('');
}
